...
class ExpectColumnValuesToBeInTypeList(ColumnMapExpectation):
    ...
    def _validate(
        ...
        if isinstance(execution_engine, PandasExecutionEngine):
            ...
            spark_result = self._validate_spark(
                actual_column_type=actual_column_type,
                expected_types_list=expected_types_list,
            )

            return_obj: Dict[str, Any] = {"success": spark_result["success"]}
            result = spark_result["result"]
            return_obj["result"] = {
                "element_count": metrics.get("table.row_count"),
                "expectation_output_metric": spark_result["success"],
            }
