...
class ExpectColumnDistinctValuesToBeInSet(ColumnAggregateExpectation):
    ...
    def _validate(
        ...
        matching_count = sum(count for value, count in observed_value_counts.items() if value in value_set)
        element_count = metrics.get("table.row_count")
        unexpected_count = element_count - matching_count
        unexpected_percent = unexpected_percent = (unexpected_count / element_count) * 100 if element_count > 0 else 0
